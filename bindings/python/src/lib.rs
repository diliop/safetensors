...
///             {"tensor_name": {"dtype": "F32", "shape": [2, 3], "data":
/// b"\0\0"}}     metadata (`Dict[str, str]`, *optional*):
...
///         The tensor dict is like:
///             {"tensor_name": {"dtype": "F32", "shape": [2, 3], "data":
/// b"\0\0"}}     filename (`str`, or `os.PathLike`):
        ...
        match &self.storage.as_ref() {
            ...
            Storage::TorchStorage(storage) => {
                Python::with_gil(|py| -> PyResult<PyObject> {
                    ...
                    // torch.asarray(storage[start + n : stop + n],
                    // dtype=torch.uint8).view(dtype=dtype).reshape(shape)
                })
            }
        }
    }
